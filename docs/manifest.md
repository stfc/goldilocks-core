...
- `write_bundle_directory(result: CoreResult, output_dir: str | Path)` writes generated files and `manifest.json` to a new bundle directory. It refuses an existing destination and has no overwrite mode.
...
Generated file paths must stay inside the bundle directory. `GeneratedFile` rejects empty, absolute, or `..`-traversing paths at construction. `write_bundle_directory` resolves each path beneath `output_dir` as a defensive filesystem check before writing, and rejects any path reserved for `manifest.json`.
