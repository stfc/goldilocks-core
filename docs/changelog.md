...
- Staged Core pipeline: Load → Analyze → Advise → Select → Generate → Bundle.
...
- `SelectionRecord` with concrete k-point grids, pseudopotential selections, and cutoff extraction.
...
- `goldilocks-core` CLI with `recommend`, `generate`, and `bundle` subcommands.
