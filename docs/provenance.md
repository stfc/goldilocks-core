...
| `model` | ML model prediction | CSLR k-index prediction → k-mesh selection |
...
- **data_source**: where the supporting data came from (e.g. model name, pseudo library, SSSP version). Populated for `model` and `lookup` sources.
- **confidence**: optional score in [0, 1]. Not currently populated by Core. Reserved for future model-backed advice.
