...
type InputData = NonNullable<ComputationResult["records"]["dft_input_data"]>;

const scientificRecords = {
  analysis: {
    formula: "Si1",
    reduced_formula: "Si",
    site_count: 1,
  ...
  advice: {
    smearing: { smearing_type: "fixed", width_ry: null, provenance },
    magnetism: { spin_polarized: false, magnetic_elements: [], provenance },
    spin_orbit: {
      enabled: false,
      consider: false,
      heavy_elements: [],
    ...
    vdw: { use_vdw: false, method: null, provenance },
  },
} satisfies ComputationResult["records"];

const pseudoIdentity: InputData["pseudopotential_set"] = {
  id: pseudoSet.id,
...
const runtime: InputData["runtime"] = {
  core_version: "1.2.3",
  models: [],
  assets: [
    {
...
const generationHints: ComputationResult["draft"]["hints"] = {
  ...capabilities.default_hints,
  k_grid: [3, 3, 3],
};
...
export const computationResult: ComputationResult = {
  ...
  publication: {
    kind: "archive",
    path: "goldilocks-inputs.zip",
    files: ["inputs/qe.in", "pseudo/Si.upf", "goldilocks.json"],
  },
  warnings: [],
  draft: {
  ...
  records: {
    ...
    dft_input_data: {
      schema_version: 1,
      pseudopotential_set: pseudoIdentity,
      artifacts: [
        { path: "inputs/qe.in", role: "input" },
        { path: "pseudo/Si.upf", role: "pseudopotential" },
      ],
      citations: [pseudoSet.citation],
      runtime,
      manifest: {
        source: { ...inspection.source, path: "source/Si.cif" },
        canonical_structure: {
          path: "structure/canonical.cif",
          metadata: inspection.structure,
        },
        intent: capabilities.default_intent,
        hints: generationHints,
        records: {
          ...scientificRecords,
          generated_files: scientificRecords.generated_files.map(
            ({ path, role }) => ({ path, role }),
          ),
        },
        pseudopotential_set: pseudoIdentity,
        ...
        citations: [pseudoSet.citation],
        files: {
          "inputs/qe.in": { sha256: "c".repeat(64), size_bytes: 11 },
          "pseudo/Si.upf": { sha256: "d".repeat(64), size_bytes: 128 },
        },
      },
    },
  },
